...
public class MyRBTree<T extends Comparable<T>> {
    ...
    private T addNode(RBTreeNode<T> node) {
        ...
        node.setRed(true);
        setParent(node, null);
        if (root.getLeft() == null) {//root左边是树的根节点。如果root左边没有就直接插到这里。
            root.setLeft(node);
            //root node is black
            node.setRed(false);
        } else {
            RBTreeNode<T> x = findParentNode(node);//如果是
    ...
    public static void main(String[] args) {
        MyRBTree<String> bst = new MyRBTree<String>();
//        bst.addNode("a");
        bst.addNode("a");
//        bst.addNode("c");
//        bst.addNode("c");
